    ...
    <div class="bg-white p-4 border-1 shadow-sm mt-4 rounded-1">
        ...
        @forelse($articles as $article)
            ...
            <p>
                ...
                <br>
                {{ $article->getAuthorList() }} {{ date('Y', strtotime($article->issue->date)) }}.
                {{ $article->title }}
                arise,
                <i>Journal of Security and Sustainability Issues</i>
                {{ $article->issue->volume }}({{ $article->issue->number }}):
        ...
        <div class="row text-center mt-3">
            <div class="col-lg-12">
                Page {{ $articles->currentPage() }} of {{ $articles->lastPage() }}, showing {{ count($articles) }}
                records
